...
- **FR-003**: System MUST automatically persist the current branch (if applicable), last-viewed file path, and raw/pretty view mode to `.gitlocal` as the user navigates, with no explicit "save settings" action required. [NEEDS CLARIFICATION: exact set of fields to persist in this first version — is it limited to last-viewed branch/path/view-mode, or should it also include things like sidebar-collapsed state and search filters that are today kept in the URL/localStorage?]
...
- **FR-012**: [NEEDS CLARIFICATION: should `.gitlocal` be committed to the repository by default, or automatically excluded from version control (e.g. via `.git/info/exclude` or a suggested `.gitignore` entry)? It may contain personal, machine-specific preferences (last file viewed, view mode) that a team may not want shared/committed by default, but the request's wording ("saved... in a .gitlocal file") doesn't specify.]
...
- Whether `.gitlocal` should be committed to version control by default or excluded is an open, product-level decision (FR-012) with real collaboration/privacy implications (personal last-viewed state vs. a possible future team-shared default view) — flagged for explicit confirmation before implementation rather than assumed.
