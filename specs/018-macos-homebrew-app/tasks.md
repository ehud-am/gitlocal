...
- [X] T062 Integrate the tag-driven release pipeline in `.github/workflows/publish.yml` so each version tag publishes npm, builds the macOS app artifact, creates the GitHub Release with the artifact attached, updates cask metadata, and pushes `Casks/gitlocal.rb` to the project-owned Homebrew tap
