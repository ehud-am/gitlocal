...
### Third round: closing out the pre-release contrarian QA findings (branch `044-pptx-qa-followups`)

Before tagging/publishing this release, the user asked to address every item flagged by the pre-release contrarian QA review documented in `releases/0.13.4-release-review.md` — the release cut that fixed 5 of those findings before merging (see that document's "Fixed before release" section) left 3 deferred, non-blocking findings on record. This round closes all 3:

- **`p:bgRef` themed backgrounds**: `parseBgFill` now resolves the second, mutually-exclusive OOXML background form — `<p:bg><p:bgRef idx="N">`, an index into the theme's `<a:fmtScheme><a:bgFillStyleLst>` (background-fill-style list) — alongside the already-supported `<p:bgPr>` direct-fill form. Added `extractApproximateFillColor` (reduces a fill-style-list entry, solid or gradient, to one approximate color) and `PptxThemeContext.bgFillStyleColor`. **Documented approximation**: OOXML's list is positionally indexed by `idx` (1-1000 → `fillStyleLst`, 1001+ → `bgFillStyleLst` using `idx-1000`), but a same-tag-name XML parse loses cross-tag-name ordering when a list mixes fill types (e.g. one `a:solidFill` + two `a:gradFill` siblings can't be told apart positionally without an order-preserving parse pass, a much larger change this implementation deliberately didn't take on) — so this resolves to the *first solid color found* in the indicated list (falling back to a gradient's first stop) rather than the exact requested index. Real-world, especially Office-derived, themes overwhelmingly put `a:solidFill` at the position most decks' `bgRef` actually references, so this is correct in the dominant practical case while being explicit in code comments about the narrower gap. New fixture: `theme-bgref.pptx`.
- **Grouped shapes (`p:grpSp`)**: added full recursive group-shape flattening. A group's own `<a:xfrm>` defines both where it sits and its children's local coordinate space (`a:chOff`/`a:chExt`); `groupChildTransform` computes the resulting affine transform (scale + offset per axis), and `composeTransforms` accumulates it through arbitrarily-nested groups. `collectGroupedShapes` recursively walks a group's `p:sp`/`p:pic`/nested-`p:grpSp` contents, applying the accumulated transform to each leaf shape and skipping hidden (`hidden="1"`) shapes/groups at any depth. Placeholders are not resolved inside groups (OOXML doesn't define grouped placeholders in practice — grouping isn't compatible with layout/master placeholder inheritance), so every shape found inside a group is treated as plain content, same as a layout/master's own non-placeholder decorations. Both `parseSlide` (a slide's own groups) and `loadPartIndex` (a layout/master's own group decorations) now handle `p:grpSp` at their top level. New fixtures: `grouped-shapes.pptx` (flat group with non-1:1 scale, nested group with composed transforms including a picture, hidden top-level group, and edge cases — no `a:xfrm` at all, no `a:chOff`/`a:chExt`, a degenerate zero-size group, hidden/empty-run/no-`txBody` shapes inside a visible group, a hidden group nested inside a visible one) and `layout-group-decoration.pptx` (a layout's own group decoration).
- **Parsing duplication and eager image loading** (the user chose the lower-risk of two options offered for the image-loading half — see below): consolidated relationship-XML parsing into one `parseRelationships` helper (previously duplicated across `findRelTarget`, `loadPartIndex`, `parseSlide`, and `parsePresentation`), extracted a shared `parsePictureShapes` helper (previously near-identical between `loadPartIndex` and `parseSlide`, now also used by `collectGroupedShapes`) that resolves a part's images via `Promise.all` instead of one at a time, merged the separate `masterCache`/`themeCache` into one `PptxMasterEntry`-keyed cache via a new `loadMasterEntry` (they were always loaded and checked together anyway), and bundled `layoutCache`+the merged master cache into one `PptxCaches` object threaded as a single parameter instead of several. Each layout/master's own `.rels` file is now read/parsed exactly once and passed to whichever of `loadThemeContext`/`loadPartIndex` needs it, instead of each independently re-reading it. **Scope decision on eager image loading**: the full fix (deferring image *decoding* until a slide's `IntersectionObserver` first fires, not just deferring shape *rendering* as today) would require retaining the parsed `zip` archive in long-lived component state so images can decode on demand later — a genuine architecture change with real component-lifecycle/memory considerations, not a pure cleanup. Offered the user a choice between that full rework and the safer partial fix (parallelizing image decoding within a part via `Promise.all`, still eager overall); the user chose the safer partial fix, which is what's implemented here.
- **Bonus, found while doing the above**: the EMF/WMF format gate in `resolveImageDataUri` was a blocklist of exactly the two formats the originally reported sample deck happened to use. Replaced it with `BROWSER_RENDERABLE_MIME_TYPES`, an allowlist of formats browsers actually decode in `<img>` — closing two gaps the blocklist missed: `.tiff` (in `extToMime`'s map but not reliably `<img>`-renderable in mainstream browsers either) and compressed vector metafiles (`.emz`/`.wmz`, not in `extToMime`'s map at all, previously falling through to a browser broken-image icon instead of the graceful "not available" placeholder). Also found and removed one genuinely dead branch while implementing `groupChildTransform`: a defensive `scale = chExt !== 0 ? ext/chExt : 1` ternary whose false side could never execute, since the `|| 1` fallback earlier in the same expression already guarantees a non-zero divisor — simplified to a plain division.

`PptxViewer.test.tsx` now at 55 cases, 91.04% branch coverage (≥90% gate maintained despite substantially more code). Full UI suite (560 tests) and full repo suite (1025 tests total) pass with 0 vulnerabilities in both `npm audit` workspaces. Re-verified against the real sample file via local server + browser after the refactor: identical rendering to before (background, hero graphic, EMF placeholder, all text, responsive scaling, keyboard navigation, scroll-spy position tracking all confirmed still working) — the parsing-consolidation and picture-loading changes are a behavior-preserving refactor, not a rendering change. Native macOS `xcodebuild` unaffected (no Swift changes this round).
