...
**Confirmed during implementation (T001–T003)**: `node-pty@1.1.0`'s published tarball bundles prebuilt binaries only for `darwin-arm64`, `darwin-x64`, `win32-arm64`, `win32-x64` — **no Linux prebuilds are shipped**. On Linux, `npm install` falls back to compiling from source via `node-gyp`, which requires Python 3 and a C++ toolchain; this failed outright in the implementation sandbox (no Python, no compiler, no root to install either). Real Linux npm users/CI runners with `build-essential`/`python3` already installed (common on GitHub Actions `ubuntu-latest`, many dev machines) will succeed, but this is a genuine install-time failure mode for Linux users without a toolchain — not merely a sandbox artifact. Tracked as a follow-up: either document a Linux system requirement (Python 3 + C++ toolchain) in the README, or migrate to a prebuild-multiarch fork with Linux binaries, before this feature reaches a release branch. Separately, confirmed `packaging/macos/release/package-app.sh` copies only `dist/`, `ui/dist/`, and `package.json` into the app bundle — with `node-pty` marked `--external` (required, since esbuild cannot bundle native addons), the Homebrew macOS app will be missing `node-pty` at runtime unless the packaging script is updated to also copy `node_modules/node-pty` (its prebuilt `darwin-*` binary) into the bundle. This packaging-script fix is scoped to the Polish phase (`tasks.md` T045), not Setup, but is now a confirmed requirement rather than a hypothetical.
