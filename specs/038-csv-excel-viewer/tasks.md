...
- [X] T001 Add `papaparse` + `@types/papaparse` (MIT) to `ui/package.json`; run `npm install` at repo root to update lockfiles
- [X] T002 Add `xlsx` (SheetJS Community Edition, Apache-2.0) to `ui/package.json`, pinned to the SheetJS-published CDN tarball URL rather than the stale npm registry copy (research.md §2, quickstart.md install note); run `npm install` and confirm the resolved version is the maintained 0.20.x line, not 0.18.5
- [X] T003 [P] Add checked-in test fixtures: `ui/src/test-fixtures/sample.csv` (simple valid CSV), `ui/src/test-fixtures/quoted-fields.csv` (quoted field containing a comma and an embedded newline), `ui/src/test-fixtures/sample.xlsx` (2+ sheets, at least one formula cell), `ui/src/test-fixtures/chart.xlsx` (embedded chart with a title), `ui/src/test-fixtures/broken.xlsx` (corrupted/truncated bytes)
...
- [X] T004 [US1] Extend `detectFileType()` in `src/git/repo.ts`: add explicit `'csv'` return (`{ type: 'csv', language: '' }`), removed from the plain-text fallback path
- [X] T005 [US2] Extend `detectFileType()` in `src/git/repo.ts`: add explicit `'excel'` return (`{ type: 'excel', language: '' }`) for `.xlsx`/`.xls`, removed from `binaryExts`
- [X] T006 (depends on T004, T005) Extend `FileContent['type']` union in `src/types.ts` to include `'csv' | 'excel'`
- [X] T007 (depends on T004, T005, T006) Add `type === 'csv'` and `type === 'excel'` branches to `fileHandler` in `src/handlers/file.ts`, per contracts/file-content-api.md — csv delivers UTF-8 text, excel delivers base64, both force `editable: false`
- [X] T008 [P] Extend `tests/unit/git/repo.test.ts`: `detectFileType('x.csv')` → `csv`/`''`; `detectFileType('x.xlsx')` and `detectFileType('x.xls')` → `excel`/`''`; confirm no other extension's classification changed
- [X] T009 [P] (depends on T007) Extend `tests/unit/handlers/file.test.ts`: new csv/excel branches return correct `encoding`/`type`/`editable`; confirm `updateFileHandler` still rejects `csv`/`excel` (existing `type !== 'markdown' && type !== 'text'` guard)
...
- [X] T010 [US1] (depends on Phase 2) Create `ui/src/components/ContentPanel/CsvViewer.tsx`: lazy-import `papaparse`, parse `content` into `CsvTable` (data-model.md), render as a scrollable `<table>` with the first row as `<th>` headers, short/ragged rows padded with empty cells
- [X] T011 [US1] (depends on T010) Handle unparseable CSV content (e.g. binary content misnamed `.csv`): render the standard "can't preview this file" fallback per FR-008; genuinely empty file renders an empty-table state, not an error
- [X] T012 [US1] (depends on T010) Wire the existing raw/pretty toggle to pass `content` directly to `CodeViewer` for the raw view (FR-003) — no second request, no re-parse
...
- [X] T014 [US1] (depends on T010) Register `csv` in `previewRegistry` (`Component: CsvViewer`, `supportsRawToggle: true`, `editable: false`) — the only change to `preview-registry.tsx` itself, zero changes to `ContentPanel.tsx` dispatch logic (SC-006)
- [X] T015 [US1] (depends on T014) Extend `ContentPanel.test.tsx` with a `csv`-type dispatch case confirming `CsvViewer` renders and no edit affordance is offered
- [X] T016 [US1] Manual verification via quickstart.md "Verify CSV preview" (incl. Network-tab check for zero external requests)
...
- [X] T017 [US2] (depends on Phase 2) Create `ui/src/components/ContentPanel/ExcelViewer.tsx`: base64-decode `content`, lazy-import `xlsx`, call `xlsx.read(data, { type: 'array' })`, build an `ExcelWorkbook` (data-model.md) from `workbook.SheetNames` and each sheet's formatted-cell grid (cached values only — never `cellFormula`/recalculation, per FR-006)
- [X] T018 [US2] (depends on T017) Render a tab strip (one tab per `workbook.SheetNames` entry, in order) above the active sheet's table; clicking a tab updates `activeSheetIndex` and swaps the visible table (FR-005)
- [X] T019 [US2] (depends on T017) Handle `xlsx.read()` throwing (corrupted, password-protected, or unsupported legacy `.xls` variant): render the standard "can't preview this file" fallback per FR-008, never a crash or indefinite loading state
...
- [X] T021 [US2] (depends on T017, T018) Register `excel` in `previewRegistry` (`Component: ExcelViewer`, `supportsRawToggle: false`, `editable: false`) — the only change to `preview-registry.tsx` itself, zero changes to `ContentPanel.tsx` dispatch logic (SC-006)
- [X] T022 [US2] (depends on T021) Extend `ContentPanel.test.tsx` with an `excel`-type dispatch case confirming `ExcelViewer` renders and no edit affordance is offered
- [X] T023 [US2] Manual verification via quickstart.md "Verify Excel preview" steps 1-4, 7-8 (chart steps covered in Phase 5)
...
- [X] T024 [US3] (depends on Phase 4 / T017) Extend `ExcelViewer.tsx`'s workbook parsing to detect each sheet's chart presence — **scope confirmed with user and revised from `ChartIndicator[]` to a `hasChart: boolean` flag** after empirically confirming SheetJS CE exposes no title/series data and no embedded-chart signal at all; only `sheet['!type'] === 'chart'` (dedicated chartsheets) is detectable (research.md §3 update)
- [X] T025 [US3] (depends on T024) Render a static labeled chart indicator on any sheet where `hasChart` is true (no title, no data reveal — neither is available); no indicator when `hasChart` is false (Acceptance Scenario 3)
- [X] T026 [P] [US3] Extend `ExcelViewer.test.tsx`: `chart.xlsx` (data sheet + dedicated chartsheet) shows the indicator only on the chartsheet tab; a chart-free fixture (`sample.xlsx`) shows no indicator on any sheet; assert no chart re-fetch/recalculation/external request occurs (Acceptance Scenario 4, SC-004)
- [X] T027 [US3] Manual verification via quickstart.md "Verify Excel preview" steps 5-6 (adjusted expectations per the confirmed scope reduction)
...
- [X] T028 [P] Run `npm run lint` (tsc --noEmit) and fix any type errors surfaced by the `FileContent['type']` union extension across the codebase (e.g. any exhaustive `switch`/`if` chains outside `ContentPanel` that assumed the prior union)
- [X] T029 [P] Run `npm test` with coverage and confirm every new/changed file (`CsvViewer.tsx`, `ExcelViewer.tsx`, `repo.ts`, `file.ts`, `preview-registry.tsx`, `ContentPanel.tsx`) meets the ≥90% per-file branch coverage gate (Constitution Principle II) — found and fixed a pre-existing gap: `CsvViewer.tsx`/`ExcelViewer.tsx` were missing from `ui/vitest.config.ts`'s coverage `include` list (so the gate was never actually being enforced on them); added both, which surfaced one truly-unreachable dead branch in `ExcelViewer.tsx` (`sheets.length === 0`, impossible via `XLSX.read` on any real workbook) that was removed rather than fake-tested
- [X] T030 [P] Run `npm run build` and confirm `papaparse`/`xlsx` are lazy-loaded (not in the initial bundle) and resolve correctly in the production build
- [X] T031 Update `CLAUDE.md` "Recent Changes" section with a `038-csv-excel-viewer` entry, following the existing entries' style
- [X] T032 Full quickstart.md walkthrough (all sections) as a final manual pass before marking the feature ready for release-branch inclusion (Constitution VIII contrarian QA remains a separate, later release-time gate) — verified server-side classification/delivery for `.csv`/`.xlsx` via direct `/api/file` requests against real fixtures (confirmed `type`/`encoding`/`content`); full click-through UI verification deferred (no browser automation available this session) but is otherwise covered by `CsvViewer.test.tsx`/`ExcelViewer.test.tsx`/`ContentPanel.test.tsx`
