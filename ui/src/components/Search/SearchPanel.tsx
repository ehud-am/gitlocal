    ...
    <section className={`search-panel${readyForResults ? ' search-panel-active' : ''}`} aria-label="repository search">
      <div className="search-panel-header">
        <div className="search-panel-heading">
          <p className="search-panel-title">Quick find repository paths</p>
          <p className="search-panel-subtitle">Type at least 3 characters to jump to a file or folder by name.</p>
        </div>
      ...
      <div className="search-controls">
        <span className="search-guidance">{readyForResults ? 'Matching files and folders' : 'Results appear after 3 characters'}</span>
        <span className="search-shortcut-hint" aria-hidden="true">Cmd/Ctrl + F</span>
      </div>
      {!readyForResults ? (
        <p className="search-empty">Type 3 or more characters to see matching files and folders.</p>
      ) : isLoading ? (
