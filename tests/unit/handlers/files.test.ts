...
describe('regular folder file handlers', () => {
  it('lists, reads, creates, updates, and deletes files in a non-git folder', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'gitlocal-regular-folder-'))
    try {
      ...
      expect(readBody.editable).toBe(true)
      expect(readBody.revisionToken).toBeTruthy()
      ...
      const newReadRes = await app.fetch(new Request(`http://localhost/api/file?path=${encodeURIComponent('docs/new.md')}`))
      const newReadBody = await newReadRes.json()
      const updateRes = await app.fetch(new Request('http://localhost/api/file', {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ path: 'docs/new.md', content: 'updated', revisionToken: newReadBody.revisionToken }),
      }))
      expect(updateRes.status).toBe(200)
      expect((await updateRes.json()).status).toBe('updated')

      const updatedReadRes = await app.fetch(new Request(`http://localhost/api/file?path=${encodeURIComponent('docs/new.md')}`))
      const updatedReadBody = await updatedReadRes.json()
      expect(updatedReadBody.content).toBe('updated')

      const deleteRes = await app.fetch(new Request('http://localhost/api/file', {
        method: 'DELETE',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ path: 'docs/new.md', revisionToken: updatedReadBody.revisionToken }),
      }))
  ...
  it('blocks regular-folder escapes, duplicates, stale writes, and missing deletes', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'gitlocal-regular-folder-errors-'))
    try {
      writeFileSync(join(dir, 'README.md'), '# Local')
      const app = createApp(dir)

      const escapedCreate = await app.fetch(new Request('http://localhost/api/file', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ path: '../escape.md', content: 'x' }),
      }))
      expect(escapedCreate.status).toBe(400)
      expect((await escapedCreate.json()).message).toContain('opened folder')

      const duplicateCreate = await app.fetch(new Request('http://localhost/api/file', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      const staleUpdate = await app.fetch(new Request('http://localhost/api/file', {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ path: 'README.md', content: 'stale', revisionToken: 'stale-token' }),
      }))
      expect(staleUpdate.status).toBe(409)

      const missingDelete = await app.fetch(new Request('http://localhost/api/file', {
        method: 'DELETE',
        headers: { 'content-type': 'application/json' },
