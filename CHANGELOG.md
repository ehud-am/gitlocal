...
- Replaced PowerPoint preview's Previous/Next button navigation with continuous scrolling through all slides, with a "Slide N of M" indicator that tracks scroll position and speaker notes shown inline per slide, and restored keyboard-only navigation (Tab now moves through every slide) that the switch to scrolling had dropped.
- Fixed PowerPoint preview also missing a slide/layout/master's themed background when set via a "Design Ideas"-style background reference (as opposed to a directly-specified color), and missing any shape a deck's author had grouped together (a common PowerPoint editing action) — both previously rendered as if the content weren't there at all.
