...
## Recent Changes
- 035-quality-hardening (interim, 0.10.3): Full architecture/code review of all 15 reviewable units (110 findings logged), plus two bug-fix checkpoints landing 12 of those findings — sync-status path-type misreporting on non-current branches, an opened-file name mismatch outside git repos, macOS app JS-injection and process-lifecycle races, missing file-tree keyboard navigation, incorrect picker `aria-expanded` state, and assorted dead-code/type-drift cleanup — plus a standalone fix pinning the file-view header above scrollable content. Dead-code removal, duplicate-code consolidation, efficiency, and readability phases (US3-US6) are not yet started — see `specs/035-quality-hardening/`.
